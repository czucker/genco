 ...
 * Handle the callback
 */
function rh_social_login_callback ()
{
	...
	if (isset ($_POST) AND !empty ($_POST ['oa_action']) AND $_POST ['oa_action'] == 'social_login' AND !empty ($_POST ['connection_token']))
	{
		...
		//API Settings
		$api_connection_handler = 'curl'; //((!empty ($settings ['api_connection_handler']) AND $settings ['api_connection_handler'] == 'fsockopen') ? 'fsockopen' : 'curl');
		$api_connection_use_https = true; //((!isset ($settings ['api_connection_use_https']) OR $settings ['api_connection_use_https'] == '1') ? true : false);

		$api_subdomain = (!empty ($settings ['api_subdomain']) ? $settings ['api_subdomain'] : '');
		$api_key = (!empty ($settings ['api_key']) ? $settings ['api_key'] : '');
		$api_secret = (!empty ($settings ['api_secret']) ? $settings ['api_secret'] : '');

		$api_resource_url = 'https://' . $api_subdomain . '.api.oneall.com/connections/' . $_POST ['connection_token'] . '.json'; //($api_connection_use_https ? 'https' : 'http') . '://' . $api_subdomain . '.api.oneall.com/connections/' . $_POST ['connection_token'] . '.json';
		...
		if (is_object ($result) AND property_exists ($result, 'http_code') AND $result->http_code == 200)
		{
			if (property_exists ($result, 'http_data'))
			{
				...
				if (is_object ($social_data))
				{
					...
					// Get user by token
					$user_id = rh_social_login_get_user_by_token ($user_token);
							...
							if (!empty ($user_email) AND $user_email_is_verified === true)
							{
								...
								if (($user_id_tmp = email_exists ($user_email)) !== false)
								{
									...
									if ($user_data !== false)
									{
										...
										delete_metadata ('user', null, 'rh_social_login_user_token', $user_token, true);
										update_user_meta ($user_id, 'rh_social_login_user_token', $user_token);
										update_user_meta ($user_id, 'rh_social_login_identity_id', $user_identity_id);
										update_user_meta ($user_id, 'rh_social_login_identity_provider', $user_identity_provider);

										if (!empty ($user_thumbnail))
										{
											update_user_meta ($user_id, 'rh_social_login_user_thumbnail', $user_thumbnail);
										}
					...
					if (!is_numeric ($user_id))
					{
						...
						//Email Restriction
						$user_email = apply_filters ('rh_social_login_filter_new_user_email', $user_email);
						if ($user_email === 'disallowed')
						{
							trigger_error (__('This Social Network account may not be used to register', 'rh_social_login'), E_USER_ERROR);
						}
						...
						{
							$user_email = rh_social_login_create_rand_email ();
						...
						if (is_numeric ($user_id))
						{
							...
							if (!empty ($settings ['plugin_require_email']))
							{
								...
								{
									update_user_meta ($user_id, 'rh_social_login_request_email', 1);
								}
							}
							...
							{
								rh_social_login_user_notification ($user_id, $user_identity_provider);
							}
					...
					if ($user_data !== false)
					{
						...
						$rh_social_login_source = (!empty ($_REQUEST ['oa_social_login_source']) ? strtolower (trim ($_REQUEST ['oa_social_login_source'])) : '');

						//Use safe redirection?
						$redirect_to_safe = false;

						//Build the url to redirect the user to
						switch ($rh_social_login_source)
						{
							//*************** Registration ***************
							case 'registration':
							//Default redirection
							...
							// *************** Comments ***************
							case 'comments':
								$redirect_to = rh_social_login_get_current_url () . '#comments';
								break;
							...
							// This is a new user
								...
								//Default value
								$redirect_to = rh_social_login_get_current_url ();
 ...
 */
function rh_social_login_do_api_request ($handler, $url, $options = array (), $timeout = 15)
{
	//FSOCKOPEN
	if ($handler == 'fsockopen')
	{
		return rh_social_login_fsockopen_request ($url, $options, $timeout);
	}
	//CURL
	else
	{
		return rh_social_login_curl_request ($url, $options, $timeout);
	}
}
 ...
 */
function rh_social_login_check_fsockopen ($secure = true)
{
	$result = rh_social_login_fsockopen_request (($secure ? 'https' : 'http') . '://www.oneall.com/ping.html');
	if (is_object ($result) AND property_exists ($result, 'http_code') AND $result->http_code == 200)
	{
		if (property_exists ($result, 'http_data'))
		{
 ...
 */
function rh_social_login_check_curl ($secure = true)
{
	if (in_array ('curl', get_loaded_extensions ()) AND function_exists ('curl_exec'))
	{
		$result = rh_social_login_curl_request (($secure ? 'https' : 'http') . '://www.oneall.com/ping.html');
		if (is_object ($result) AND property_exists ($result, 'http_code') AND $result->http_code == 200)
		{
			if (property_exists ($result, 'http_data'))
			{
 ...
 */
function rh_social_login_curl_request ($url, $options = array (), $timeout = 10)
{
	//Store the result
	$result = new stdClass ();
	...
	curl_setopt ($curl, CURLOPT_USERAGENT, 'SocialLogin ' . RH_SOCIAL_LOGIN_VERSION . 'WP (+http://www.oneall.com/)');
 ...
 */
function rh_social_login_fsockopen_request ($url, $options = array (), $timeout = 15)
{
	//Store the result
	$result = new stdClass ();
	...
	$defaults = array (
		'Host' => "Host: $host",
		'User-Agent' => 'User-Agent: SocialLogin ' . rh_SOCIAL_LOGIN_VERSION . 'WP (+http://www.oneall.com/)'
	);
