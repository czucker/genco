...
class OTW_Grid_Manager extends OTW_Component{
	...
	public function __construct(){
		...
		add_shortcode( 'otw_shortcode_grid_row', array( &$this, 'otw_shortcode_grid_row' ) );
	...
	public function action_meta_boxes(){
		...
		switch( $object_post_type ){
			...
			case 'post':
			case 'page':
					if( isset( $this->active_for[ $object_post_type ] ) && $this->active_for[ $object_post_type ] ){
						$register_meta = true;
					}
		...
		if( $register_meta && $show_meta ){
			add_meta_box( $this->meta_name, $this->get_label('OTW Grid Manager'), array( &$this, 'build_meta_box' ), '', 'normal', 'high');
		}
	}
	...
	/**
	 * Check if the component is active for current object
	 */
	public function is_active_for( $object )
	{
		if( isset( $object->post_type ) ){
			
			if( array_key_exists( $object->post_type, $this->active_for ) && !$this->active_for[ $object->post_type ] ){
				return false;
			}
		}
		return true;
