...
function init_manage_page(){
	...
	var s_labels = jQuery( '.otw_sidebar_item_filter label' ).click(  function( event ){
		event.preventDefault();
		otw_select_sitem( this );
	} );
	
	jQuery( '.otw_sbm_global_manage_settings a' ).click( function(){
		
		var action = '';
		
		switch( jQuery( this ).attr( 'class' ) ){
		
			case 'otw_all_global_manage_vis':
					action = 'all';
				break;
			case 'otw_all_global_manage_invis':
					action = 'none';
				break;
		}
		
		jQuery( '.sitems' ).each( function(){
			...
			var item_type = '';
			if( b_matches = box.attr( 'id' ).match( /^otw_sbm_type_(.*)$/ ) ){
			
				item_type = b_matches[1];
			}
			...
			if( item_type != 'userroles' ){
				...
				switch( action ){
					...
					case 'all':
							box.removeClass( 'otw_closed' );
							box.parents( 'div.postbox' ).removeClass( 'closed' );
							
							box.find( 'a.otw_sbm_select_all_items' ).click();
						break;
					case 'none':
							box.find( 'a.otw_sbm_unselect_all_items' ).click();
						break;
				}
			}
		} );
...
function otw_sbm_man_filter_wp_items( search_box ){
	...
	var bar = search_box.parents( '.sitems' );
